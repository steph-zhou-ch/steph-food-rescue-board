# Composed worker prompt — `w1-api-items`

> Agent class: `application-services-agent` (Scion template: `application-services-agent`)
> Wave / batch: w1 / batch 1 · Phase 4
...
End-to-end capability slice — composes domain + application use cases + persistence + integrations + the apps/-resident inbound surface into a coherent feature (e.g. app-book-appointment).
...
**Track id:** `w1-api-items`

**Summary:**

Implement the full REST API for surplus items: POST (create), GET
(list with category filter), PATCH (claim/unclaim/confirm-pickup),
and DELETE (soft-remove). Uses an in-memory data store — no database
required. Includes the SurplusItem domain entity, status state machine,
and all acceptance criteria from the API REQs.

**Predecessors:**
- (none — ready immediately)

**Deliverables** (files you will create or modify):
- `libs/domain/src/surplus-item.ts`
- `libs/domain/src/item-status.ts`
- `libs/domain/src/index.ts`
- `libs/domain/test/surplus-item.spec.ts`
- `libs/application/src/item-store.ts`
...
- `apps/api/src/items/dto/update-status.dto.ts`
- `apps/api/src/app.module.ts`
- `apps/api/test/items.spec.ts`

**Exit criterion:**

All 4 API endpoints respond correctly, the status state machine rejects
illegal transitions with 409, req-lint + typecheck + vitest all pass,
and every critical/high criterion from REQ-CAP-POST-ITEM,
REQ-CAP-BROWSE-FEED, REQ-CAP-CLAIM-ITEM, REQ-CAP-REMOVE-LISTING, and
REQ-INV-ITEM-LIFECYCLE has a tagged test.
...
**Branch.** Work on `swarm/w1-api-items`. Pull from `origin/main` at start; rebase only if your manager pushes a fix-batch.
...
**Done marker.** When all listed deliverables are complete and `pnpm typecheck && pnpm test` passes on `swarm/w1-api-items`, commit `[complete:w1-api-items]` (subject only) and push. The manager polls `origin/swarm/w1-api-items` for this marker.

**Push policy.** You push to `origin/swarm/w1-api-items` only. The manager merges to staging and then to `main`. Do NOT push to `main` or to a sibling track's branch.
