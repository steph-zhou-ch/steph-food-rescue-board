# agents.md — foundations-agent (Claude operating instructions)
...
2. For each `@criterion` in your assigned REQ, in topological order:
   a. Write the failing test in `the relevant test/ dir per your foundation track-meta/<area>.spec.ts`.
   ...
   d. Author the minimum impl in `the relevant src/ dir per your foundation track-meta/<area>.ts`.
   e. `pnpm typecheck && pnpm typecheck && pnpm test (from repo root; foundation work is cross-package)` — both must pass.
