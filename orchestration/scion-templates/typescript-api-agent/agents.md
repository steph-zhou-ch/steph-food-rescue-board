# agents.md — typescript-api-agent (Claude operating instructions)
...
2. For each `@criterion` in your assigned REQ, in topological order:
   a. Write the failing test in `apps/app/test/<area>.spec.ts`.
   ...
   d. Author the minimum impl in `apps/app/src/<area>.ts`.
   e. `pnpm typecheck && pnpm --filter @charliehealth/app test` — both must pass.
