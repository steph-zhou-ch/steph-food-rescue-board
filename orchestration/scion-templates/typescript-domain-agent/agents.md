# agents.md — typescript-domain-agent (Claude operating instructions)
...
2. For each `@criterion` in your assigned REQ, in topological order:
   a. Write the failing test in `libs/domain/test/<area>.spec.ts`.
   ...
   d. Author the minimum impl in `libs/domain/src/<area>.ts`.
   e. `pnpm typecheck && pnpm --filter @charliehealth/domain test` — both must pass.
