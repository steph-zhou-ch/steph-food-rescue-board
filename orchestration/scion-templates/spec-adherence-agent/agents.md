# agents.md — spec-adherence-agent (Claude operating instructions)
...
2. For each `@criterion` in your assigned REQ, in topological order:
   a. Write the failing test in `orchestration/reviews/ (your verdict file)/<area>.spec.ts`.
   ...
   d. Author the minimum impl in `(read-only on the whole repo; write only to orchestration/reviews/)/<area>.ts`.
   e. `pnpm typecheck && pnpm test (only if helpful — your verdict is not about whether tests pass, but whether the predicate is satisfied)` — both must pass.
